 ...
 */
    ...
    public Mono<ResponseEntity<Object>> process(@RequestBody AuthRequest authRequest, Authentication authentication){
        ...
        else if (authRequest.getPassword() == null || authRequest.getName() == null) return Mono.just(ResponseEntity.unprocessableEntity().build());
        else return userServiceImpl.checkIfUsernameExists(authRequest.getName())
                .map(isExist->{
                    if (isExist) {
